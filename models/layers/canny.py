...
class Canny(nn.Module):
    ...
    def __init__(self, threshold=10.0, filter_size=5, dtype=torch.float32):
        super().__init__()
        self.threshold = threshold
        self.filter_size = filter_size

        generated_filters = gaussian(filter_size, std=1.0).reshape([1, filter_size])
        gf = torch.from_numpy(generated_filters).type(dtype)

        self.gaussian_filter_horizontal = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(1, filter_size), padding=(0, filter_size // 2))
        self.gaussian_filter_horizontal.weight.data.copy_(gf)
        nn.init.constant_(self.gaussian_filter_horizontal.bias, 0.0)
        self.gaussian_filter_vertical = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(filter_size, 1), padding=(filter_size // 2, 0))
        self.gaussian_filter_vertical.weight.data.copy_(gf.T)
        nn.init.constant_(self.gaussian_filter_vertical.bias, 0.0)

        sobel_filter = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]])
        sf = torch.from_numpy(sobel_filter).type(dtype)

        self.sobel_filter_horizontal = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=sobel_filter.shape, padding=sobel_filter.shape[0] // 2)
        self.sobel_filter_horizontal.weight.data.copy_(sf)
        nn.init.constant_(self.sobel_filter_horizontal.bias, 0.0)
        self.sobel_filter_vertical = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=sobel_filter.shape, padding=sobel_filter.shape[0] // 2)
        self.sobel_filter_vertical.weight.data.copy_(sf.T)
        nn.init.constant_(self.sobel_filter_vertical.bias, 0.0)
        ...
        self.directional_filter.weight.data.copy_(torch.from_numpy(all_filters[:, None, ...]).type(dtype))
        nn.init.constant_(self.directional_filter.bias, 0.0)
    ...
    def forward(self, img):
        ...
        grad_orientation = (torch.atan2(grad_y_r + grad_y_g + grad_y_b, grad_x_r + grad_x_g + grad_x_b) * (180.0 / np.pi))
...
class Canny2D(nn.Module):
    ...
    def forward(self, img):
        horizontal = self.gaussian_filter_horizontal(img)
        blurred_img = self.gaussian_filter_vertical(horizontal)
