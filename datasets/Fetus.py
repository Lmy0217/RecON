...
class Fetus(datasets.BaseDataset):
        ...
        for idx in range(count_actions):
            ...
            if action == 'line':
                ...
                while line_series is None:
                    ...
                    length = 6 * np.random.rand() + 16
                    end_point = start_points[0, :] + direct * length
                    start_velocity = direct * (1.0 + 0.1 * np.random.randn()) + 0.05 * torch.randn((3,), dtype=direct.dtype, device=self.cfg.device)
                    start_accele = 0
                ...
                while line_series is None:
                    if while_time >= self.cfg.while_max_time:
                        return None
                    while_time += 1
                    length = 40 * np.random.rand() + 120
                    end_point = end_point + direct * length
                    start_velocity = direct * (12 + 4.0 * np.random.randn()) + 0.05 * torch.randn((3,), dtype=direct.dtype, device=self.cfg.device)
                    start_accele = 0
                    line_series = utils.simulation.sample_line(series[-1], end_point, start_velocity, start_accele, radius=self.rad, n=n, min_n=nn - 1)
                line_series = line_series[:nn - 1, ...]
            ...
            while ii_loop < loops:
                ...
                if while_broke_flag:
                    ...
                    if while_broke_count2 >= self.cfg.while_max_time:
                        ...
                        ii_loop = 0
                        ss_loop = None
                        random_point = utils.simulation.sample_points_at_border(n, height=self.cfg.source.height, width=self.cfg.source.width, radius=self.rad, direct_down=self.direct_down)
                        points[ii] = random_point[0, ...]
                    continue
